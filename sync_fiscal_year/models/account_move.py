...
class AccountMove(models.Model):
    ...
    def create(self, vals):
        period_id = False
        if vals.get('date') and not vals.get('period_id') and vals.get('type') in ['entry']:
        ...
        if vals.get('invoice_date') and not vals.get('period_id') and vals.get('type') not in ['entry']:
            period_id = self.env['account.period'].search([('state', '!=', 'done'), ('special', '=', False), ('date_start', '<=', vals.get('invoice_date')), ('date_stop', '>=', vals.get('invoice_date')), ('company_id', '=', self.env.user.company_id.id)], limit=1)
        if period_id:
    ...
    def write(self, vals):
        period_id = False
        if vals.get('date') and vals.get('type') in ['entry']:
            period_id = self.env['account.period'].search([('state', '!=', 'done'), ('date_start', '<=', vals.get('date')), ('date_stop', '>=', vals.get('date')), ('company_id', '=', self.env.user.company_id.id)], limit=1)
        if vals.get('invoice_date') and vals.get('type') not in ['entry']:
            period_id = self.env['account.period'].search([('state', '!=', 'done'), ('special', '=', False),
                                                           ('date_start', '<=', vals.get('invoice_date')), ('date_stop', '>=', vals.get('invoice_date')), ('company_id', '=', self.env.user.company_id.id)], limit=1)
        if period_id:
            vals.update({'period_id': period_id.id})
    ...
    def _onchange_date(self):
        period_id = False
        if self.date and self.type in ['entry']:
        ...
        if self.invoice_date and self.type not in ['entry']:
            self.period_id = False
            period_id = self.env['account.period'].search([('state', '!=', 'done'), ('special', '=', False),
                                                           ('date_start', '<=', self.invoice_date), ('date_stop', '>=', self.invoice_date), ('company_id', '=', self.env.user.company_id.id)], limit=1)
        if period_id:
    ...
    def _centralise(self, mode):
        ...
        if res:
            ...
            line_id = account_move_line_obj.with_context({'check_move_validity': False}).create({
