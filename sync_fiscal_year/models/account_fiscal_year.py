...
class AccountFiscalyear(models.Model):
    ...
    def set_period(self):
        for payment in self.env['account.payment'].search([('payment_date', '>=', self.date_start), ('payment_date', '<=', self.date_stop), ('period_id', '=', False)]):
            period_id = self.env['account.period'].search([('special', '=', False),
        ...
        for move in self.env['account.move'].search(['|', ('invoice_date', '>=', self.date_start), ('invoice_date', '<=', self.date_stop),
...
class AccountPeriod(models.Model):
    ...
    special = fields.Boolean('Opening/Closing Period', help="These periods can overlap.", default=False)
