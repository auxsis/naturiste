...
class AccountFiscalYearClose(models.TransientModel):
    ...
    journal_id = fields.Many2one('account.journal', 'Opening Entries Journal', domain="[('type','=','situation')]",
                                 required=True, help='The best practice here is to use a journal dedicated to contain the opening entries of all fiscal years. Note that you should define it with default debit/credit accounts, of type \'situation\' and with a centralized counterpart.')
    ...
    def data_save(self):
        ...
        for account in obj_acc_account.browse(account_ids):
            ...
            for aml in self.env['account.move.line'].search([('account_id', '=', account.id),('state', '=', 'posted'), ('date', '>=', self.fy_id.date_start), ('date', '<=', self.fy_id.date_stop)]):
            ...
            if (abs(balance)) > 0:
                ...
                query_2nd_part_args += (balance > 0 and balance or 0.0,
                       ...
                       account.balance if account.currency_id else 0.0,
