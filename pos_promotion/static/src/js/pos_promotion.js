...
odoo.define('pos_promotion', function (require) {
    ...
    models.load_models([
        {
            ...
            fields: ['name', 'start_date', 'end_date', 'type', 'product_id', 'discount_lowest_price', 'product_ids'],
    ...
    models.Order = models.Order.extend({
        ...
        checking_buy_x_get_another_free: function (promotion) {
            ...
            if (lines_will_apply_promotion.length) {
                can_apply = true;
            }
            return can_apply;
        },
            ...
            */
            ...
            if (discount_line_tmp && total_order > 0) {
                ...
                if (product && price != 0) {
                    ...
                    options.promotion_reason = 'Applied ' + promotion['name'] + ', discount ' + discount_line_tmp.discount + ' % ' + ' because total order greater or equal ' + discount_line_tmp.minimum_amount;
            ...
            for (var i in this.pos.promotion_by_category_id) {
                ...
                if (amount_total_by_category != 0) {
                    ...
                    this.add_promotion(product, price, 1, {
                        ...
                        promotion: true,
                        promotion_reason: 'Applied ' + promotion['name'] + ', discount ' + promotion_line.discount + ' % from ' + promotion_line.category_id[1],
                    })
                }
            }
        },
                        ...
                        if (price_item_tmp) {
                            ...
                            while (z < lines.length) {
                                ...
                                if (line.product.id == price_item_tmp.product_id[0]) {
                                    if (this.pos.config.iface_tax_included === 'total') {
                                        price_discount += (line.get_price_with_tax() / line.quantity - price_item_tmp.list_price) * line.quantity
                                    } else {
                                        price_discount += (line.get_price_without_tax() / line.quantity -  price_item_tmp.list_price) * line.quantity
                                    }
                                }
                                z++;
                            }
                            if (price_discount != 0) {
                                this.add_promotion(product, price_discount, -1, {
                                    ...
                                    promotion_reason: 'Applied ' + promotion['name'] + ', By greater or equal ' + price_item_tmp.minimum_quantity + ' ' + price_item_tmp.product_id[1] + ' applied new price ' + price_item_tmp.list_price
                                })
                            }
                        }
                    }
        ...
        current_order_can_apply_promotion: function () {
            ...
            for (var i = 0; i < this.pos.promotions.length; i++) {
                ...
                if (promotion['type'] == '1_discount_total_order' && this.checking_apply_total_order(promotion)) {
                    ...
                    can_apply = this.checking_buy_x_get_another_free(promotion);
                    if (can_apply) {
                        promotions_apply.push(promotion);
                    }
                }
    ...
    screens.ActionpadWidget.include({
        /*
                validation payment
                auto ask cashiers need apply promotion
         */
